...
- **Refatoração e reorganização do código** — Em execução via `docs/refactoring-plan.md`. Separar `css/style.css` em três camadas (base/components/pages), reorganizar JS por feature, introduzir `content/` e `authoring/` como taxonomias de alto nível, navbar sticky, responsividade básica (mobile-aware). Ver plano para detalhamento dos blocos A–F
