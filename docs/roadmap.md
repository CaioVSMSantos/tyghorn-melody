...
- [x] Módulo 3: Notas Musicais (3 tópicos — 12 notas, tom/semitom, Dó central)
...
- **Revisão textual e factual do Módulo de Teoria Musical** — Revisão aprofundada, feita por partes de forma focada, para garantir (a) veracidade factual das informações apresentadas (datas, atribuições históricas, afirmações teóricas, etimologias), (b) concisão e clareza do texto, (c) consistência terminológica entre tópicos e (d) qualidade gramatical e de estilo. A revisão acontecerá tópico a tópico, com foco direcionado a cada rodada, e deverá anteceder a declaração de "conclusão" de qualquer fase de conteúdo.
  - **Progresso (2026-05-28):** Fase B revisada por completo — tópicos 1.1 a 3.3 auditados (veracidade factual, poda de travessões, convenção de callouts `info`/`tip`/`warn`, consistência terminológica). Correções factuais notáveis: tabela de Helmholtz (Dó central = c′, estava deslocada uma oitava), atribuição indevida "Yamaha/Roland" do C3 (a Roland usa C4), século de Doni (XVII), monofonia × monodia, Michelangelo Rossi (compositor, não autor de método), datas não verificáveis de Seymour Fink. Módulos 4+ pendentes de revisão à medida que forem escritos.
